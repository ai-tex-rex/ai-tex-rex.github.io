...
class ConclusionAnalyzer:
    def __init__(self, api_key: str, 
                 analysis_template: str = 'conclusion/conclusion_prompt_coll/analysis_template.md',
                 total_template: str = 'conclusion/conclusion_prompt_coll/total_template.md',
                 conclusion_data: str = 'conclusion/conclusion_data.json'
    ):
        self.api_key = api_key
        self.analysis_template = self._load_template(analysis_template)
        self.total_template = self._load_template(total_template)
        self.conclusion_data = self._load_json(conclusion_data)
    ...
    def create_markdown_section(self, target_title_indx: int) -> str:
        if target_title_indx >= len(self.conclusion_data):
            return "Section not found"
            
        section = self.conclusion_data[target_title_indx]
    ...
    async def analyze_conclusion(self, conclusion: str, model: str = "anthropic/claude-3.5-sonnet") -> str:
        # Generate prompts collection
        g_prompt_coll = []
        for indx in range(len(self.conclusion_data)):
            guidelines = self.create_markdown_section(indx)
            t_ = self.analysis_template.format(user_conclusion=conclusion, guidelines=guidelines)
        ...
        tot_text = ""
        for i in out_coll:
            tot_text += self.format_improvement_instructions(i[1]['conclusion_improvement_instructions'])
            tot_text += "-"*50 + "\n"

        self.analysis_results = results
        self.analysis_results_condensed = tot_text
        # Generate final analysis
        final_prompt = self.total_template.format(user_conclusion=conclusion, tot_results=tot_text)
