...
class IntroductionAnalyzer:
    def __init__(self, api_key: str, 
                 analysis_template: str = 'introduction/intro_prompt_coll/analysis_template.md',
                 total_template: str = 'introduction/intro_prompt_coll/total_template.md',
                 introduction_data: str = 'introduction/introduction_data.json'
    ):
        self.api_key = api_key
        self.analysis_template = self._load_template(analysis_template)
        self.total_template = self._load_template(total_template)
        self.introduction_data = self._load_json(introduction_data)
    ...
    def create_markdown_section(self, target_title_indx: int) -> str:
        if target_title_indx >= len(self.introduction_data):
            return "Section not found"
            
        section = self.introduction_data[target_title_indx]
    ...
    async def analyze_introduction(self, introduction: str, model: str = "anthropic/claude-3.5-sonnet") -> str:
        # Generate prompts collection
        g_prompt_coll = []
        for indx in range(len(self.introduction_data)):
            guidelines = self.create_markdown_section(indx)
            t_ = self.analysis_template.format(user_introduction=introduction, guidelines=guidelines)
        ...
        tot_text = ""
        for i in out_coll:
            tot_text += self.format_improvement_instructions(i[1]['introduction_improvement_instructions'])
            tot_text += "-"*50 + "\n"

        self.analysis_results = results
        self.analysis_results_condensed = tot_text
        # Generate final analysis
        final_prompt = self.total_template.format(user_introduction=introduction, tot_results=tot_text)
