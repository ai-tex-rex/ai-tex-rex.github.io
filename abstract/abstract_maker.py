...
class AbstractAnalyzer:
    def __init__(self, api_key: str, 
                 analysis_template: str = 'abstract/prompt_coll/analysis_template.md',
                 total_template: str = 'abstract/prompt_coll/total_template.md',
                 abstract_data: str = 'abstract/abstract_data.json'
    ):
        self.api_key = api_key
        self.analysis_template = self._load_template(analysis_template)
        self.total_template = self._load_template(total_template)
        self.abstract_data = self._load_json(abstract_data)
    ...
    def create_markdown_section(self, target_title_indx: int) -> str:
        if target_title_indx >= len(self.abstract_data):
            return "Section not found"
            
        section = self.abstract_data[target_title_indx]
    ...
    async def analyze_abstract(self, abstract: str, model: str = "anthropic/claude-3.5-sonnet") -> str:
        # Generate prompts collection
        g_prompt_coll = []
        for indx in range(len(self.abstract_data)):
            guidelines = self.create_markdown_section(indx)
            t_ = self.analysis_template.format(user_abstract=abstract, guidelines=guidelines)
        ...
        tot_text = ""
        for i in out_coll:
            tot_text += self.format_improvement_instructions(i[1]['abstract_improvement_instructions'])
            tot_text += "-"*50 + "\n"

        self.analysis_results = results
        self.analysis_results_condensed = tot_text
        # Generate final analysis
        final_prompt = self.total_template.format(user_abstract=abstract, tot_results=tot_text)
